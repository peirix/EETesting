jQuery(document).ready(function($) {
	...
	$("#ml_trigger, .ml_trigger_inline").live('click',function() {
	...
	$("#ml_dialog a:not([class=nofb])").live('click', function() {
		$link = $(this);
	...
	// correct the Switchboard field width for webkit browsers
	// (why? because webkit has a fancy type="search" appearance that ignores padding, so if we
	// don't do this we end up with a field that's too thin
	// update: disabling rounded corners now too, as Chrome for Windows gets confused
	if ($.browser.webkit) {
		if($search_field.length > 0)
		{
			$search_field.width('214px !important')
							.css('border-radius', '0 !important')
							.css('-webkit-border-radius', '0 !important')
							.css('-webkit-border-radius', '0 !important')
