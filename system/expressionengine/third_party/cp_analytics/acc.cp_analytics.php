...
class Cp_analytics_acc {
	...
	function set_sections()
	{
		...
		else
		{
			...
			$today['hourly_updated'] = $this->EE->localize->decode_date('%g:%i%a', $today['cache_time']);
			...
			// If we're using the Republic theme, and not caching our sparklines, we can change the sparkline color
			if($this->theme == 'republic')
			{
				$find = 'FFFFFF';
				$replace = 'A9C526';
				$daily['lastmonth']['visits_sparkline'] = str_replace($find, $replace, $daily['lastmonth']['visits_sparkline']);
				$daily['lastmonth']['pageviews_sparkline'] = str_replace($find, $replace, $daily['lastmonth']['pageviews_sparkline']);
				$daily['lastmonth']['pages_per_visit_sparkline'] = str_replace($find, $replace, $daily['lastmonth']['pages_per_visit_sparkline']);
				$daily['lastmonth']['bounce_rate_sparkline'] = str_replace($find, $replace, $daily['lastmonth']['bounce_rate_sparkline']);
				$daily['lastmonth']['avg_visit_sparkline'] = str_replace($find, $replace, $daily['lastmonth']['avg_visit_sparkline']);
				$daily['lastmonth']['new_visits_sparkline'] = str_replace($find, $replace, $daily['lastmonth']['new_visits_sparkline']);
			}
