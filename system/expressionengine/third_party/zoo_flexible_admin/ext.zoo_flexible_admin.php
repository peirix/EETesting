		...
		if($zoo_nav){
			...
			if($this->nav != ""){	
				...
				$js .= '
					...
					$("#navigationTabs #modulefolder > a:first-child").attr("href", "'.$_SERVER["SCRIPT_NAME"].'?D=cp&C=addons_modules");
					...
					if(foundin.length > 0){
						...
						bc ="<li><a href=\"'.$_SERVER["SCRIPT_NAME"].'?D=cp&C=homepage\">'.$this->EE->config->item('site_name').'</a></li>";	
		...
		if ($query->num_rows() > 0)
		{
			if(in_array("homepage", $_GET) && $query->row()->startpage != "" && strlen($query->row()->startpage) > 3){
				...
				if(strpos($prev, "C=homepage") === false){
					...
					header("Refresh: 0;url=".$query->row()->startpage);
					exit;
				}
			}
